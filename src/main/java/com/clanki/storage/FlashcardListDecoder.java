...
public class FlashcardListDecoder {
    ...
    public static final Pattern FLASHCARD_ARGS_FORMAT = Pattern.compile("q/(?<question>[^/]+)"
            + " a/(?<answer>[^/]+)" + " d/(?<dueDate>[^/]+)" + " p/(?<currentPeriod>[^/]+)");
    ...
    /**
     * Decodes {@code encodedFlashcardList} into an {@code ArrayList<Flashcard>}
